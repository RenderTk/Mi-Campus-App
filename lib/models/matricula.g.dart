// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'matricula.dart';

// **************************************************************************
// CopyWithGenerator
// **************************************************************************

abstract class _$MatriculaCWProxy {
  Matricula idSeccion(int? idSeccion);

  Matricula codigoCurso(String? codigoCurso);

  Matricula descripcionCurso(String? descripcionCurso);

  Matricula creditos(int? creditos);

  Matricula dias(String? dias);

  Matricula inicio(DateTime? inicio);

  Matricula fin(DateTime? fin);

  Matricula aula(String? aula);

  Matricula numeroSeccion(int? numeroSeccion);

  Matricula periodo(int? periodo);

  Matricula cupos(int? cupos);

  Matricula cuposModalidad(int? cuposModalidad);

  Matricula hibrida(int? hibrida);

  Matricula estaSeleccionada(int? estaSeleccionada);

  Matricula nombreCompleto(String? nombreCompleto);

  Matricula optativa(int? optativa);

  Matricula modalidad(String? modalidad);

  Matricula idDetallePlan(int? idDetallePlan);

  Matricula hdr(int? hdr);

  /// Creates a new instance with the provided field values.
  /// Passing `null` to a nullable field nullifies it, while `null` for a non-nullable field is ignored. To update a single field use `Matricula(...).copyWith.fieldName(value)`.
  ///
  /// Example:
  /// ```dart
  /// Matricula(...).copyWith(id: 12, name: "My name")
  /// ```
  Matricula call({
    int? idSeccion,
    String? codigoCurso,
    String? descripcionCurso,
    int? creditos,
    String? dias,
    DateTime? inicio,
    DateTime? fin,
    String? aula,
    int? numeroSeccion,
    int? periodo,
    int? cupos,
    int? cuposModalidad,
    int? hibrida,
    int? estaSeleccionada,
    String? nombreCompleto,
    int? optativa,
    String? modalidad,
    int? idDetallePlan,
    int? hdr,
  });
}

/// Callable proxy for `copyWith` functionality.
/// Use as `instanceOfMatricula.copyWith(...)` or call `instanceOfMatricula.copyWith.fieldName(value)` for a single field.
class _$MatriculaCWProxyImpl implements _$MatriculaCWProxy {
  const _$MatriculaCWProxyImpl(this._value);

  final Matricula _value;

  @override
  Matricula idSeccion(int? idSeccion) => call(idSeccion: idSeccion);

  @override
  Matricula codigoCurso(String? codigoCurso) => call(codigoCurso: codigoCurso);

  @override
  Matricula descripcionCurso(String? descripcionCurso) =>
      call(descripcionCurso: descripcionCurso);

  @override
  Matricula creditos(int? creditos) => call(creditos: creditos);

  @override
  Matricula dias(String? dias) => call(dias: dias);

  @override
  Matricula inicio(DateTime? inicio) => call(inicio: inicio);

  @override
  Matricula fin(DateTime? fin) => call(fin: fin);

  @override
  Matricula aula(String? aula) => call(aula: aula);

  @override
  Matricula numeroSeccion(int? numeroSeccion) =>
      call(numeroSeccion: numeroSeccion);

  @override
  Matricula periodo(int? periodo) => call(periodo: periodo);

  @override
  Matricula cupos(int? cupos) => call(cupos: cupos);

  @override
  Matricula cuposModalidad(int? cuposModalidad) =>
      call(cuposModalidad: cuposModalidad);

  @override
  Matricula hibrida(int? hibrida) => call(hibrida: hibrida);

  @override
  Matricula estaSeleccionada(int? estaSeleccionada) =>
      call(estaSeleccionada: estaSeleccionada);

  @override
  Matricula nombreCompleto(String? nombreCompleto) =>
      call(nombreCompleto: nombreCompleto);

  @override
  Matricula optativa(int? optativa) => call(optativa: optativa);

  @override
  Matricula modalidad(String? modalidad) => call(modalidad: modalidad);

  @override
  Matricula idDetallePlan(int? idDetallePlan) =>
      call(idDetallePlan: idDetallePlan);

  @override
  Matricula hdr(int? hdr) => call(hdr: hdr);

  @override
  /// Creates a new instance with the provided field values.
  /// Passing `null` to a nullable field nullifies it, while `null` for a non-nullable field is ignored. To update a single field use `Matricula(...).copyWith.fieldName(value)`.
  ///
  /// Example:
  /// ```dart
  /// Matricula(...).copyWith(id: 12, name: "My name")
  /// ```
  Matricula call({
    Object? idSeccion = const $CopyWithPlaceholder(),
    Object? codigoCurso = const $CopyWithPlaceholder(),
    Object? descripcionCurso = const $CopyWithPlaceholder(),
    Object? creditos = const $CopyWithPlaceholder(),
    Object? dias = const $CopyWithPlaceholder(),
    Object? inicio = const $CopyWithPlaceholder(),
    Object? fin = const $CopyWithPlaceholder(),
    Object? aula = const $CopyWithPlaceholder(),
    Object? numeroSeccion = const $CopyWithPlaceholder(),
    Object? periodo = const $CopyWithPlaceholder(),
    Object? cupos = const $CopyWithPlaceholder(),
    Object? cuposModalidad = const $CopyWithPlaceholder(),
    Object? hibrida = const $CopyWithPlaceholder(),
    Object? estaSeleccionada = const $CopyWithPlaceholder(),
    Object? nombreCompleto = const $CopyWithPlaceholder(),
    Object? optativa = const $CopyWithPlaceholder(),
    Object? modalidad = const $CopyWithPlaceholder(),
    Object? idDetallePlan = const $CopyWithPlaceholder(),
    Object? hdr = const $CopyWithPlaceholder(),
  }) {
    return Matricula(
      idSeccion: idSeccion == const $CopyWithPlaceholder()
          ? _value.idSeccion
          // ignore: cast_nullable_to_non_nullable
          : idSeccion as int?,
      codigoCurso: codigoCurso == const $CopyWithPlaceholder()
          ? _value.codigoCurso
          // ignore: cast_nullable_to_non_nullable
          : codigoCurso as String?,
      descripcionCurso: descripcionCurso == const $CopyWithPlaceholder()
          ? _value.descripcionCurso
          // ignore: cast_nullable_to_non_nullable
          : descripcionCurso as String?,
      creditos: creditos == const $CopyWithPlaceholder()
          ? _value.creditos
          // ignore: cast_nullable_to_non_nullable
          : creditos as int?,
      dias: dias == const $CopyWithPlaceholder()
          ? _value.dias
          // ignore: cast_nullable_to_non_nullable
          : dias as String?,
      inicio: inicio == const $CopyWithPlaceholder()
          ? _value.inicio
          // ignore: cast_nullable_to_non_nullable
          : inicio as DateTime?,
      fin: fin == const $CopyWithPlaceholder()
          ? _value.fin
          // ignore: cast_nullable_to_non_nullable
          : fin as DateTime?,
      aula: aula == const $CopyWithPlaceholder()
          ? _value.aula
          // ignore: cast_nullable_to_non_nullable
          : aula as String?,
      numeroSeccion: numeroSeccion == const $CopyWithPlaceholder()
          ? _value.numeroSeccion
          // ignore: cast_nullable_to_non_nullable
          : numeroSeccion as int?,
      periodo: periodo == const $CopyWithPlaceholder()
          ? _value.periodo
          // ignore: cast_nullable_to_non_nullable
          : periodo as int?,
      cupos: cupos == const $CopyWithPlaceholder()
          ? _value.cupos
          // ignore: cast_nullable_to_non_nullable
          : cupos as int?,
      cuposModalidad: cuposModalidad == const $CopyWithPlaceholder()
          ? _value.cuposModalidad
          // ignore: cast_nullable_to_non_nullable
          : cuposModalidad as int?,
      hibrida: hibrida == const $CopyWithPlaceholder()
          ? _value.hibrida
          // ignore: cast_nullable_to_non_nullable
          : hibrida as int?,
      estaSeleccionada: estaSeleccionada == const $CopyWithPlaceholder()
          ? _value.estaSeleccionada
          // ignore: cast_nullable_to_non_nullable
          : estaSeleccionada as int?,
      nombreCompleto: nombreCompleto == const $CopyWithPlaceholder()
          ? _value.nombreCompleto
          // ignore: cast_nullable_to_non_nullable
          : nombreCompleto as String?,
      optativa: optativa == const $CopyWithPlaceholder()
          ? _value.optativa
          // ignore: cast_nullable_to_non_nullable
          : optativa as int?,
      modalidad: modalidad == const $CopyWithPlaceholder()
          ? _value.modalidad
          // ignore: cast_nullable_to_non_nullable
          : modalidad as String?,
      idDetallePlan: idDetallePlan == const $CopyWithPlaceholder()
          ? _value.idDetallePlan
          // ignore: cast_nullable_to_non_nullable
          : idDetallePlan as int?,
      hdr: hdr == const $CopyWithPlaceholder()
          ? _value.hdr
          // ignore: cast_nullable_to_non_nullable
          : hdr as int?,
    );
  }
}

extension $MatriculaCopyWith on Matricula {
  /// Returns a callable class used to build a new instance with modified fields.
  /// Example: `instanceOfMatricula.copyWith(...)` or `instanceOfMatricula.copyWith.fieldName(...)`.
  // ignore: library_private_types_in_public_api
  _$MatriculaCWProxy get copyWith => _$MatriculaCWProxyImpl(this);
}

// **************************************************************************
// JsonSerializableGenerator
// **************************************************************************

Matricula _$MatriculaFromJson(Map<String, dynamic> json) => Matricula(
  idSeccion: (json['ID_SECCION'] as num?)?.toInt(),
  codigoCurso: json['CODIGO_CURSO'] as String?,
  descripcionCurso: json['DESCRIPCION_CURSO'] as String?,
  creditos: (json['CREDITOS'] as num?)?.toInt(),
  dias: json['DIAS'] as String?,
  inicio: json['INICIO'] == null
      ? null
      : DateTime.parse(json['INICIO'] as String),
  fin: json['FIN'] == null ? null : DateTime.parse(json['FIN'] as String),
  aula: json['AULA'] as String?,
  numeroSeccion: (json['NUMERO_SECCION'] as num?)?.toInt(),
  periodo: (json['PERIODO'] as num?)?.toInt(),
  cupos: (json['DISPONIBLE'] as num?)?.toInt(),
  cuposModalidad: (json['DISPONIBLE_MODALIDAD'] as num?)?.toInt(),
  hibrida: Matricula._hibridaFromJson(json['HIBRIDA']),
  estaSeleccionada: (json['CHECKED'] as num?)?.toInt(),
  nombreCompleto: json['NOMBRE_COMPLETO'] as String?,
  optativa: (json['OPTATIVA'] as num?)?.toInt(),
  modalidad: json['MODALIDAD'] as String?,
  idDetallePlan: (json['ID_DETALLE_PLAN'] as num?)?.toInt(),
  hdr: (json['HDR'] as num?)?.toInt(),
);

Map<String, dynamic> _$MatriculaToJson(Matricula instance) => <String, dynamic>{
  'ID_SECCION': instance.idSeccion,
  'CODIGO_CURSO': instance.codigoCurso,
  'DESCRIPCION_CURSO': instance.descripcionCurso,
  'CREDITOS': instance.creditos,
  'DIAS': instance.dias,
  'INICIO': instance.inicio?.toIso8601String(),
  'FIN': instance.fin?.toIso8601String(),
  'AULA': instance.aula,
  'NUMERO_SECCION': instance.numeroSeccion,
  'PERIODO': instance.periodo,
  'DISPONIBLE': instance.cupos,
  'DISPONIBLE_MODALIDAD': instance.cuposModalidad,
  'HIBRIDA': instance.hibrida,
  'CHECKED': instance.estaSeleccionada,
  'NOMBRE_COMPLETO': instance.nombreCompleto,
  'OPTATIVA': instance.optativa,
  'MODALIDAD': instance.modalidad,
  'ID_DETALLE_PLAN': instance.idDetallePlan,
  'HDR': instance.hdr,
};
